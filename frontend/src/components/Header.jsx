...
export default function Header() {
  ...
  const navClass = ({ isActive }) =>
    `relative text-lg font-semibold transition duration-300 ${
      isActive
        ? "text-sky-400 after:absolute after:-bottom-1 after:left-0 after:w-full after:h-[2px] after:bg-sky-400"
    ...
    return (
      <header className="bg-gradient-to-r from-slate-900 to-slate-700 text-white shadow-lg">
        <div className="container mx-auto px-6 py-5 flex items-center justify-between flex-wrap gap-4">
          <Link to="/admin" className="text-3xl font-bold tracking-wide">
            Book Fair Admin
          </Link>
  ...
  return (
    <header className="bg-gradient-to-r from-slate-900 to-slate-700 text-white shadow-lg">
      <div className="container mx-auto px-6 py-5 flex items-center justify-between flex-wrap gap-4">
        <Link to="/" className="text-3xl font-bold tracking-wide">Book Fair</Link>
        ...
        <nav className="flex items-center gap-8 flex-wrap">
          ...
          {!user && (
            <>
              <NavLink
                to="/register"
                className="text-lg font-semibold text-stone-200 hover:text-sky-300 transition"
              >
                Create Account
              </NavLink>
              <NavLink
                ...
                className={({ isActive }) =>
                  `px-6 py-2 text-lg rounded-lg font-semibold transition ${
                    isActive ? "bg-white text-slate-900" : "bg-sky-500 hover:bg-sky-600 text-white"
                  }`
                }
              >
                Login
